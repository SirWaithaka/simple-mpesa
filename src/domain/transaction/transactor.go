...
func (tr transactor) deposit(source, destination TxnCustomer, amount value_objects.Shillings) error {
	...
	if source.UserType == value_objects.UserTypSuperAgent && destination.UserType != value_objects.UserTypAgent {
		return errors.Error{Code: errors.EINVALID, Message: errors.SuperAgentCantDeposit}
	}

	// a merchant is not allowed to deposit
	if destination.UserType == value_objects.UserTypMerchant {
...
func (tr transactor) withdraw(source, destination TxnCustomer, amount value_objects.Shillings) error {
	...
	if destination.UserType == value_objects.UserTypSuperAgent || source.UserType == value_objects.UserTypSuperAgent {
		return errors.Error{Code: errors.EINVALID, Message: errors.SuperAgentCantWithdraw}
	}

	// the destination should always be an agent
	if destination.UserType != value_objects.UserTypAgent {
	...
	// get the charge applicable to this transaction
	charge, err := tr.tariff.GetCharge(value_objects.TxnOpWithdraw, source.UserType, destination.UserType)
	if err != nil {
		return err
	}
	...
	srcNewBal, err := tr.accountant.DebitAccount(source.UserID, amt, value_objects.TxnOpWithdraw)
	if err != nil {
		return err
	}

	destNewBal, err := tr.accountant.CreditAccount(destination.UserID, amount.ToCents(), value_objects.TxnOpWithdraw)
	if err != nil {
		return err
	}
...
func (tr transactor) transfer(source, destination TxnCustomer, amount value_objects.Shillings) error {
	...
	if source.UserType == value_objects.UserTypSuperAgent || destination.UserType == value_objects.UserTypSuperAgent {
		return errors.Error{Code: errors.EINVALID, Message: errors.SuperAgentCantTransfer}
	}

	// get the charge applicable to this transaction
	charge, err := tr.tariff.GetCharge(value_objects.TxnOpTransfer, source.UserType, destination.UserType)
	if err != nil {
		return err
	}
	...
	srcNewBal, err := tr.accountant.DebitAccount(source.UserID, amt, value_objects.TxnOpTransfer)
	if err != nil {
		return err
	}

	destNewBal, err := tr.accountant.CreditAccount(destination.UserID, amount.ToCents(), value_objects.TxnOpTransfer)
	if err != nil {
		return err
	}
