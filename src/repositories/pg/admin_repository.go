...
func (r AdminRepository) searchBy(search schema.Admin) (administrator.Admin, error) {
	var row schema.Admin
...
func (r AdminRepository) Add(admin administrator.Admin) (administrator.Admin, error) {
	...
	result := r.db.Model(schema.Admin{}).Create(&row)
	if err := result.Error; err != nil {
...
func (r AdminRepository) Update(admin administrator.Admin) error {
	...
	result := r.db.Model(&schema.Admin{}).Where(&schema.Admin{ID: admin.ID}).Omit("id").Updates(row)
	if err := result.Error; err != nil {
		return errors.Error{Err: result.Error, Code: errors.EINTERNAL}
	}
	return nil
